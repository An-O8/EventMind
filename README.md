...
Six modules work together as a single-page app:
...
**Session Feedback** - Star rating and free text per session with a live character counter. In production this pipes to Google Sheets via Apps Script.
...
```bash
git clone https://github.com/An_O8/eventmind
cd eventmind
npm install
```
...
- Feedback is client side only in this demo. Production version would POST to Google Sheets via Apps Script.
...
- Firebase Realtime DB for live announcements
